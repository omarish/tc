...
func TestDecodeUTF8ChangesTokenCount(t *testing.T) {
	...
	rawCount, err := countTokens(string(raw))
	if err != nil {
		t.Fatal(err)
	}
	...
	decodedCount, err := countTokens(decoded)
	if err != nil {
		t.Fatal(err)
	}
	...
	if decodedCount != 2 {
		t.Errorf("decoded count = %d, want 2", decodedCount)
	}
	if rawCount == decodedCount {
		t.Errorf("counting raw bytes gave the same answer (%d); this test no longer proves anything", rawCount)
	}
}
